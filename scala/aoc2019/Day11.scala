...
object Day11 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var row: Int = 0
    var col: Int = 0
    var direction: Int = 0 // 0 is north, 1 is east, 2 is south, 3 is west
    var start: Boolean = true
    while (!robot.isFinished) {
      var input: Int = 0
      if (paintedWhite.contains(Panel(row, col)) || start) {
        input = 1
      }
      val output: ArrayBuffer[Long] = robot.exe(Array(input))
      if (output(0) == 0) {
        paintedBlack += Panel(row, col)
        paintedWhite -= Panel(row, col)
      } else {
        paintedWhite += Panel(row, col)
        paintedBlack -= Panel(row, col)
      }
      if (output(1) == 1) { // right 90
    ...
    for (r <- 0 to 5) {
      for (c <- 0 to 40) {
        if (paintedWhite.contains(Panel(r, c))) {
          print('#')
        } else print('.')
      }
      println
